...
const DIVIDER_STORAGE_KEY = "autograder_feedback_divider_pct";
...
export default function FeedbackDetailPage() {
  ...
  // reportRef targets the left-pane content for PDF export
  ...
  useEffect(() => {
    ...
    async function fetchSubmission() {
      ...
      setError("");
      try {
        const resp = await fetch(`${API}/api/autograder/submissions/${submissionId}`, {
          headers: { Authorization: `Bearer ${getToken()}` },
        });
        if (!resp.ok) throw new Error((await resp.text()) || "Failed to fetch submission.");
        setSubmission(await resp.json());
      } catch (err) {
    ...
    return (
      <div className="flex h-screen items-center justify-center bg-slate-50 p-6 text-center">
        <div className="max-w-sm">
          ...
          <a href="/autograder/feedbacks" className="mt-4 inline-block text-sm text-blue-600 hover:underline">
  ...
  // ── Feedback content (captured by reportRef for PDF export) ───────────────
  const feedbackContent = (
    <div ref={reportRef} className="px-6 py-5 space-y-5">
      <div>
        ...
        <p className="mt-0.5 text-sm text-slate-600">NetID / Code: {submission.student_net_id}</p>
        ...
        {result.details && (
          <p className="mt-0.5 text-xs text-slate-400">
            Model: {result.details.model} · Provider: {result.details.source}
          </p>
        )}
      </div>
