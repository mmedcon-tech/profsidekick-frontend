      ...
      <div className="fixed bottom-6 right-6 z-50 w-80 rounded-2xl shadow-2xl overflow-hidden border border-gray-700 bg-gray-900 flex flex-col max-h-[85vh]">
        ...
        <div className="flex items-center gap-2 px-3 py-2.5 bg-gray-800 border-t border-gray-700 flex-shrink-0">
          <input
            ...
            className="flex-1 bg-gray-700 text-white text-xs rounded-lg px-3 py-2 placeholder-gray-500 focus:outline-none focus:ring-1 focus:ring-emerald-500 dark:focus:ring-emerald-500"
          />
          <button
            onClick={sendMessage}
            disabled={!chatInput.trim() || isSending}
            className="w-8 h-8 rounded-lg bg-emerald-600 dark:bg-emerald-700 hover:bg-emerald-700 dark:hover:bg-emerald-600 text-white flex items-center justify-center disabled:opacity-40 disabled:cursor-not-allowed transition-colors flex-shrink-0"
            aria-label="Send"
          >
            <Send size={13} />
          </button>
        ...
        {!isConnected && (
          <div className="absolute inset-0 flex flex-col items-center justify-center bg-gray-900">
            {connectionError ? (
              ...
              <>
                <div className="w-8 h-8 border-2 border-emerald-500 dark:border-emerald-500 border-t-transparent rounded-full animate-spin mb-3" />
      ...
      <div className="flex items-center justify-center py-3 bg-gray-800 border-t border-gray-700">
        <button
          ...
          className={`flex items-center gap-2 px-5 py-2 rounded-full text-sm font-medium transition-all disabled:opacity-40 disabled:cursor-not-allowed ${
            isMicActive ? 'bg-red-600 hover:bg-red-700 text-white' : 'bg-emerald-600 dark:bg-emerald-700 hover:bg-emerald-700 dark:hover:bg-emerald-600 text-white'
          }`}
        >
