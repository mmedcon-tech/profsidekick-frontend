...
export default function ClassCreation() {
  ...
  return (
    <ProtectedRoute>
      <div className="min-h-screen bg-gradient-to-br from-[#133221] to-[#1a442d] flex items-center justify-center p-4">
        ...
        <div className="w-full max-w-3xl bg-white dark:bg-gray-800 rounded-2xl shadow-xl p-8">
          <div className="text-center mb-8">
            <div className="flex items-center justify-center gap-4 mb-4">
              ...
              <h1 className="text-3xl font-bold text-[#133221]">
                Create New Session
              </h1>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <fieldset className="space-y-4 p-4 border border-gray-200 dark:border-gray-700 rounded-lg">
              <legend className="text-lg font-semibold text-[#d4af37] px-2">
                  ...
                  <button
                    ...
                    className={`flex flex-col items-start gap-1 p-4 rounded-xl border-2 text-left transition-colors ${
                      sessionMode === 'teaching'
                        ? 'border-[#BA984E] bg-[#133221]/5'
                        : 'border-gray-200 dark:border-gray-700 hover:border-gray-300 hover:bg-gray-50 dark:bg-gray-900'
                    }`}
                  >
                    <span className={`text-sm font-semibold ${sessionMode === 'teaching' ? 'text-[#d4af37]' : 'text-gray-700 dark:text-gray-300'}`}>
                      📚 Teaching Mode
                    </span>
                ...
                <div>
                  ...
                  <div className="space-y-2">
                    {availableRoles.map((role) => (
                      <label key={role.id} className={`flex items-start gap-3 p-3 border rounded-lg cursor-pointer transition-colors ${selectedRoleId === role.id ? 'border-[#BA984E] bg-[#133221]/5' : 'border-gray-200 dark:border-gray-700 hover:border-gray-300'}`}>
                        <input
                          type="radio"
                          name="session_role"
                          value={role.id}
            ...
            <button
              type="submit"
              disabled={isLoading}
              className="w-full bg-[#133221] text-white py-3 px-4 rounded-lg font-medium hover:bg-[#1a442d] focus:ring-2 focus:ring-[#BA984E] focus:ring-offset-2 disabled:opacity-50 disabled:cursor-not-allowed transition-colors text-base"
            >
              {isLoading ? (
