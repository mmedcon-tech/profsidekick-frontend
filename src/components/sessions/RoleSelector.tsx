              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-3 py-2.5 flex items-start gap-2.5 hover:bg-gray-50 dark:bg-gray-900 transition-colors ${
                    isSelected ? 'bg-[#133221]/5' : ''
                  }`}>
                  <div className={`w-4 h-4 rounded-full border flex items-center justify-center flex-shrink-0 mt-0.5 ${
                    isSelected ? 'bg-[#133221] border-[#BA984E]' : 'border-gray-300'
                  }`}>
                    {isSelected && <Check size={10} className="text-white" />}
                  </div>
                  <div className="min-w-0">
                    <p className={`text-sm font-medium leading-tight ${isSelected ? 'text-[#d4af37]' : 'text-gray-900 dark:text-gray-100'}`}>
                      {role.name}
                    </p>
                    {role.description && (
