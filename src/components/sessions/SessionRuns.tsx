...
export default function SessionRuns({ sessionId, courseId, onRunClick }: SessionRunsProps) {
  ...
  const getStatusIcon = (status: string) => {
    switch (status) {
      ...
      case 'ACTIVE':    return <Play className="w-4 h-4 text-emerald-700 dark:text-emerald-400" />;
  ...
  const getStatusColor = (status: string) => {
    switch (status) {
      ...
      case 'ACTIVE':    return 'bg-emerald-100 dark:bg-emerald-900/40 text-emerald-900 dark:text-emerald-200';
  ...
  if (error) {
    return (
      <div className="text-center py-6">
        ...
        <button onClick={() => refetch()} className="text-emerald-700 dark:text-emerald-400 hover:text-emerald-800 dark:text-emerald-300 text-sm font-medium">
          Try again
        </button>
      </div>
    );
      ...
      {runs.map((run: any) => (
        <div
          ...
          className={`p-4 border border-gray-200 dark:border-gray-700 rounded-lg transition-all duration-200 ${
            run.status === 'ACTIVE'
              ? 'hover:border-emerald-300 dark:border-emerald-600 hover:bg-emerald-50 dark:bg-emerald-900/20 cursor-pointer'
        ...
        >
          ...
          {run.status === 'ACTIVE' && (
            <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-800 flex items-center gap-2 text-emerald-700 dark:text-emerald-400 text-sm font-medium">
